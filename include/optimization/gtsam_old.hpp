...
public:
    ...
    GTSAM() { 
        ...
        pubImuPath = nh.advertise<nav_msgs::Path>("/imu_path", 20);
    }
    ...
    bool initialize(const State& init_state) {
        ...
        imuIntegratorOpt_ = std::move(imuIntegrator);    
    ...
    void addImuFactor(const SensorMeasurement& mea) {
        ...
        // FIXME: test imu odometry
        optimize(1);
        publishPath(pubImuPath, pubImuOdometry, ImuOdomPath, prevPose_, ros::Time(last_imu_time), "map", "base");
